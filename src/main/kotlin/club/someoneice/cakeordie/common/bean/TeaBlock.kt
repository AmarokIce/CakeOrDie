...
open class TeaBlock: Block(Properties.copy(Blocks.GLASS).strength(0.02F).lightLevel { 8 }.noOcclusion()) {
    ...
    open class CakeBlockItem(block: Block, properties: Properties = Properties()): BlockItem(block, properties) {
        override fun getUseAnimation(item: ItemStack): UseAnim = UseAnim.DRINK
    }
}